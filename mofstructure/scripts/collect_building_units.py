...
def compile_data(cif_files, result_folder, verbose=False):
    ...
    guest molecule present in the MOF. After that it computes the porosity
    of all the MOFs and loads them into a single csv. Finally it deconstructs
    the MOFs into the various building units and creates three JSON files:

    1. ase_atoms_building_units.json
       JSON file containing ASE atom objects of all the building units and for
       each ASE atom object there is additional information in the info[] key.

    2. sbus_and_linkers.json
       A JSON file containing all the information about the linkers and metal SBUs.

    3. ligands.json
       A JSON file containing all the information about the ligands and metal clusters.

    Parameters
    ----------
    cif_files : list
        A list of CIF files (or any ASE-readable file) containing MOFs.
    result_folder : str
